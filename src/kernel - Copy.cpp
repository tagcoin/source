...
// Copyright (c) 2013-2015 The TagCoin developers
...
bool CheckStakeKernelHash(uint32_t nBits, const CBlock& blockFrom, uint32_t nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, uint32_t nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    if (!GetKernelStakeModifier(blockFrom.GetHash(), nStakeModifier, nStakeModifierHeight, nStakeModifierTime, fPrintProofOfStake)){
	    printf("Error getting stake modifier?\n");
        return false;}
    ss << nStakeModifier;
    ...
    // Now check if proof-of-stake hash meets target protocol. Only enforced after block 20000 due to a bug.
    if (CBigNum(hashProofOfStake) > bnCoinDayWeight * bnTargetPerCoinDay){
	    printf("Does not meet target protocol\n");
        return false;}
