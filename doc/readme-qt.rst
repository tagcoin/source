TagCoin-qt: Qt4 GUI for TagCoin
===============================
...
Alternatively, install Qt Creator and open the `TagCoin-qt.pro` file.

An executable named `TagCoin-qt` will be built.
...
A warning for people using the *static binary* version of TagCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of TagCoin is linked against libdb5.3.
...
installed causes TagCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
